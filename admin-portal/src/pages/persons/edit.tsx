...
import { useAdminPerson, useUpdatePerson } from "@/lib/api";
...
export default function PersonEdit() {
  ...
  const { data: personWrapper } = useAdminPerson(id);
  ...
  useEffect(() => {
    if (person) {
      setName(person.name);
      setBio(person.bio || "");
      setProfileImage(person.profile_image);
    }
  }, [person]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    updatePerson.mutate({ id, data: { name, bio, profile_image: profileImage || undefined } });
  };
  ...
  return (
    <Layout>
      <div className="flex items-center gap-6 mb-6">
        ...
        <h1 className="text-xl font-bold">Edit person</h1>
      </div>
      ...
      >
        ...
        <div className="pt-4 border-t border-[#2F3336]">
          <Button 
            type="submit" 
            disabled={updatePerson.isPending || !name || isUploading}
